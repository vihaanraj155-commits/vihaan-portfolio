import { AtSign, BookOpen } from "lucide-react";
import type { ComponentType } from "react";

import type { SocialIcon, SocialLink } from "@/lib/types";

/**
 * lucide-react v1 dropped brand marks, and substituting a generic globe for GitHub looks
 * wrong. These are the official brand glyphs, inlined so there is no extra dependency and
 * no external request.
 */
...
function BrandIcon({ size = 17, path, label }: IconProps & { path: string; label: string }) {
  return (
    <svg
      role="img"
      aria-hidden
...
const ICONS: Record<SocialIcon, ComponentType<IconProps>> = {
  github: (props) => <BrandIcon {...props} path={GITHUB_PATH} label="GitHub" />,
  linkedin: (props) => <BrandIcon {...props} path={LINKEDIN_PATH} label="LinkedIn" />,
  x: (props) => <BrandIcon {...props} path={X_PATH} label="X" />,
  email: ({ size = 17 }) => <AtSign size={size} aria-hidden />,
  scholar: ({ size = 17 }) => <BookOpen size={size} aria-hidden />,
};

export function SocialIcons({ socials }: { socials: SocialLink[] }) {
  return (
        ...
        return (
          <li key={social.url}>
            <a
              ...
              className="grid h-10 w-10 place-items-center rounded-full border border-hairline text-muted transition-all duration-200 hover:-translate-y-px hover:border-hairline-strong hover:text-ink"
            >
              <Icon size={17} />
            </a>
          </li>
